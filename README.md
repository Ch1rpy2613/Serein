...
天津单城、时间驱动的触感天气图集。底层天空常驻；上层按场景切换温度 / 降水 / 风 / 湿度 / 空气 / 雷达，并支持垂直剖面、分析模式（探空 · 对比）。

## 功能清单

- **感受模式**：温度、降水、风、湿度、空气 + 雷达地图图标入口
- **分析模式**：同场景分析叠加 + 探空（Skew-T）+ 多模式对比
...
- **性能**：全局 fps 降级（`PerformanceGovernor` → 全场景 `setQuality`）；探空拖轴 30fps 上限、松手补绘；历史加载 >8s 骨架
